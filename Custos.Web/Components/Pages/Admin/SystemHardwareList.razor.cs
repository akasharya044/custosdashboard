...
{
    public partial class SystemHardwareList
    {

        Variant variant = Variant.Filled;
        public IBrowserFile? SelectedFile { get; private set; }
        List<HardwareInformationDto> _hardwarelist { get; set; } = new List<HardwareInformationDto>();
        ...
        {
            var output = await custosapiservice.GetData("/api/systemInformation/gethardwarebyuserid?user=" + user);

            if (output != null && output.Status == "Success")
            {
                _hardwarelist = JsonConvert.DeserializeObject<List<HardwareInformationDto>>(output.Data.ToString());
                _hardwarelist = _hardwarelist.DistinctBy(x => x.Name).ToList();
            }
        ...
        private async Task DownloadCsvHardware()
        {
            try
            {
                ...
                var csvContent = GenerateCsvContentinstalled(_hardwarelist);

                // Prepare the file content for download
                var data = Encoding.UTF8.GetBytes(csvContent);

                // Initiate the file download using FileSaver
                await JSRuntime.InvokeVoidAsync("saveAsFile", user + " hardwarelist Activity.csv", data, "text/csv");
        ...
        private string GenerateCsvContentinstalled(List<HardwareInformationDto> data)
        {
            try
            {
